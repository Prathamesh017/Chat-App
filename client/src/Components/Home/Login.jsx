...
const checkForErrors=function(){
    if(!data.email || !data.password){
        setError((err)=>({...err,showToast:true,showToastStatus:"error"}))
    }
    else{
        login();
    }
...
<FormControl sx={{ width: '100%' }}>
  <OutlinedInput placeholder="Please enter password" style={{color:"black"}} onChange={(event)=>{setData((existingData)=>({...existingData,password:event.target.value}))}} />
</FormControl>

<Button variant="contained" style={{width:"100%" ,marginTop:"40px"}} onClick={checkForErrors}>Login</Button>
